      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          {[
            ...
            <div key={item.label} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex gap-4 hover:border-purple-500/50 transition-colors">
              <span className="text-sm text-zinc-500 min-w-[140px]">{item.label}</span>
              {item.link ? (
                <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-purple-400 hover:underline">{item.value}</a>
              ) : (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-20">
          ...
          {education.map((edu, i) => (
            <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 hover:border-purple-500/50 transition-colors">
              <div className="flex flex-wrap justify-between gap-3 mb-2">
                <h3 className="text-xl font-semibold text-purple-300">{edu.degree}</h3>
                <span className="text-xs font-medium px-3 py-1.5 bg-purple-600/20 text-purple-400 border border-purple-500/30 rounded-full">{edu.period}</span>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-20">
          ...
          <div className="space-y-12">
            {categories.map(({ label, color }) => {
              ...
              return (
                <div key={label}>
                  <h3 className={`text-sm font-semibold uppercase tracking-widest mb-5 bg-gradient-to-r ${color} bg-clip-text text-transparent`}>{label}</h3>
                  <div className="space-y-3">
                    {items.map((skill, i) => (
                      <motion.div key={skill.name} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
