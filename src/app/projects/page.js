          ...
          {allTags.map((tag) => (
            <button key={tag} onClick={() => setActive(tag)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
                active === tag ? "bg-purple-600 text-white" : "border border-zinc-700 text-zinc-400 hover:border-purple-500 hover:text-purple-400"
              }`}>
              {tag}
            </button>
          ))}
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-1">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    {project.featured && (
                      <span className="shrink-0 text-xs px-2 py-1 bg-purple-600/20 text-purple-400 border border-purple-500/30 rounded-full">Featured</span>
                    )}
                  </div>
