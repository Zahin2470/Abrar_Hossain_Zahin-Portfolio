...
const categories = [
  { label: "AI/ML",  color: "from-purple-500 to-pink-500",   glow: "rgba(168,85,247,0.15)" },
  { label: "Web",    color: "from-blue-500 to-cyan-500",     glow: "rgba(59,130,246,0.15)"  },
  { label: "DevOps", color: "from-emerald-500 to-teal-500",  glow: "rgba(16,185,129,0.15)"  },
];
    ...
    >
      <div className="flex justify-between items-center mb-1.5">
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-zinc-600 w-4">{String(index + 1).padStart(2, "0")}</span>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
              ...
              <div key={i} className="relative mb-6">
                ...
                <div className="relative bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 rounded-2xl p-6 transition-colors">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-zinc-400">
                      <span className="text-zinc-600 font-mono text-xs">loc.</span>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {categories.map(({ label, color, glow }) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 rounded-2xl p-6 transition-colors h-full">
                    {/* Category header */}
                    <div className="flex items-center gap-3 mb-6">
                      <div className={`w-1 h-8 rounded-full bg-gradient-to-b ${color}`} />
                      <div>
                        <p className={`text-xs font-mono font-bold uppercase tracking-widest bg-gradient-to-r ${color} bg-clip-text text-transparent`}>
                          {label}
                        </p>
                        <p className="text-xs text-zinc-600 font-mono">{items.length} skills</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      {items.map((skill, i) => (
                        <SkillBar key={skill.name} skill={skill} color={color} index={i} />
                      ))}
                    </div>
                  </div>
                </motion.div>
