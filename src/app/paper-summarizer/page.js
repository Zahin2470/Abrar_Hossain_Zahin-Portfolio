      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <div className="flex items-center gap-3 mt-4 flex-wrap">
            <span className="text-xs font-mono text-zinc-600">Free · No account needed · Instant results</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <span className="text-xs font-mono text-zinc-600 self-center">Try example:</span>
            {EXAMPLES.map((ex) => (
              <button key={ex.label} onClick={() => loadExample(ex.text)}
                className="text-xs font-mono px-3 py-1.5 border border-zinc-800 hover:border-purple-500/50 hover:bg-purple-500/10 text-zinc-500 hover:text-purple-400 rounded-lg transition-all duration-200">
                {ex.label}
              </button>
            ))}
          </div>
